...
const GameStatements = ({ packName, socket }) => {
	...
	const handleSendMessage = (statement) => {
		if (statement.trim() && localStorage.getItem('userName')) {
		  socket.emit('message', {
			text: statement,
	...
	useEffect(() => {
		...
		// Llama a handleSendMessage con el primer enunciado cuando se cargan los enunciados
		if (shuffledStatements.length > 0) {
			handleSendMessage(shuffledStatements[0]);
		}
	...
	const nextStatement = () => {
		if (currentStatement < statements.length - 1) {
			handleSendMessage(statements[currentStatement]);
			setCurrentStatement(currentStatement + 1);
			setBorderColor(generateRandomColor());
		}
	};

	const statement = statements.length === 0 ? statements[currentStatement] : statements[currentStatement - 1];
